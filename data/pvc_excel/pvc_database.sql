...
--PIPE PRICE QUERY
--@block
SELECT pvc_size_imperial,
    pvc_type,
    pipe_price
FROM pvc_sizes AS ps
    INNER JOIN pipe_prices AS pp ON ps.pvc_size_id = pp.pvc_size_id
    INNER JOIN pvc_types AS pt USING (pvc_type_id)
WHERE pvc_size_metric = '63mm'
ORDER BY pipe_price ASC --@block
    --PIPE PRICE QUERY
    --@block
SELECT pvc_size_imperial,
    pvc_type,
...
FROM pvc_sizes AS ps
    INNER JOIN pipe_prices AS pp ON ps.pvc_size_id = pp.pvc_size_id
    INNER JOIN pvc_types AS pt ON pt.pvc_type_id = pp.pvc_type_id
WHERE pvc_size_metric = '110mm'
ORDER BY pipe_price ASC --@block
    --FITTING PRICE QUERY
    --@block
SELECT fitting_type,
    ...
    pvc_type,
    fitting_price
FROM fitting_types AS ft
    INNER JOIN fitting_prices AS fp ON ft.fitting_type_id = fp.fitting_type_id
    INNER JOIN pvc_types AS pt USING (pvc_type_id)
    INNER JOIN pvc_sizes AS ps USING (pvc_size_id)
WHERE fitting_type = 'bend'
    AND pvc_size_metric = '110mm' --@block
    --USE DATAABASE  
    --@block
    use materials;
--@block
